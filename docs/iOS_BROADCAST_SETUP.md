...
4. オプション入力:
   ...
   - **Organization Identifier**: `com.example.localAudioSync`(あるいは独自のドメイン)
   - **Bundle Identifier**: 自動で `com.example.localAudioSync.BroadcastExtension` になる
   - **Language**: Swift
   - **Project**: Runner
...
5. **「+」** ボタンで新規 App Group を作成、ID は `group.com.example.local_audio_sync` を入力(チェックを入れる)。
...
4. 既に作成した `group.com.example.local_audio_sync` に **チェック** を入れる。
...
<string>com.example.localAudioSync.BroadcastExtension</string>
```
