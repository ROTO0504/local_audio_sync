...
1. **Runner ターゲット → Signing & Capabilities** で自分の Apple ID(Team)を選ぶ。Bundle ID は `com.example.localAudioSync` のままで実機テストは可能。
2. **「+ Capability」→ App Groups** を追加し、`group.com.example.local_audio_sync` を有効化。
...
- Bundle ID が他人と衝突している場合は `com.example.localAudioSync` を自分のドメインに変更
